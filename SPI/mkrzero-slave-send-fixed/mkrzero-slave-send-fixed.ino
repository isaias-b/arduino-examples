...
/// GLOBAL VARIABLES
char msg[1024];                 // buffers a message sent via serial
bool isWithinTx = false;        // syncs loop and ISR to avoid data races
bool needsUpdate = false;       // determines if triggering an update is required
bool shouldPrintSerial = true;  // determines if msg should be printed via serial
void (*update)(MODEL&) = updateRangeBit9; // callback function pointer
uint8_t atBuffer = 0;
MODEL buffers[3];
...
void loop() {
  ...
  measureData(data);
  buffers[2] = data;
  if(!isWithinTx) buffers[atBuffer] = buffers[2];
  preloadData(buffers[atBuffer]);
}

void measureData(MODEL &model) {
...
void preloadData(MODEL &model) {
  static int counter = 0;
  if(isWithinTx) return;
  atBuffer = nextBuffer(atBuffer);
}

uint8_t nextBuffer(uint8_t at) {
  return (at + 1) % 2;
}

void printSerial(MODEL &model) {
  if(!shouldPrintSerial) return;
  format(model, msg);
  Serial.println(msg);
}
...
void MY_SERCOM_HANDLER() {
  ...
  uint8_t data = 0;
  static uint8_t bufferNumber = nextBuffer(atBuffer);
  ...
  {
    isWithinTx = true;
    bufferNumber = nextBuffer(atBuffer);
    bytesPos = offset;
  ...
  if(intflag.bit.DRE)
  {
    MY_SERCOM->SPI.DATA.reg = buffers[bufferNumber].bytes[bytesPos++ % NBYTES]; //send byte
  }
